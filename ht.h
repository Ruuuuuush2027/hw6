...
{
    if(load >= CAPACITIES[mIndex_] * alpha)
        resize();
...
{
    std::vector<HashItem*> oldtbl_ = table_;
    table_ = std::vector<HashItem*>();
    mIndex_ ++;
    count = 0;
    load = 0;
    ...
    for(HASH_INDEX_T i = 0; i < oldtbl_.size(); i ++){
        if(oldtbl_[i] == nullptr)
            continue;
        
        if(!oldtbl_[i]->deleted)
            insert(oldtbl_[i]->item);
